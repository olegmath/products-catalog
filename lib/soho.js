...
// Thrown when the Cloudflare Turnstile check fails — handlers turn it into a 400 with a user message.
...
// No-op when TURNSTILE_SECRET_KEY is unset (local dev) — otherwise verifies the token with Cloudflare.
async function verifyTurnstile(token, remoteIp) {
  const secret = turnstileSecret();
  if (!secret) return;
  if (!token || typeof token !== "string") throw new TurnstileError("turnstile missing");
  ...
  try {
    ...
    throw new TurnstileError("turnstile unreachable");
  }
  if (!data.success) throw new TurnstileError("turnstile failed");
}
