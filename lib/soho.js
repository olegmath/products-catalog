...
const PROMO_CODES = {
  МАЙ500: 500, // фиксированная скидка 500 ₽
};
...
async function verifyTurnstile(token, remoteIp) {
  ...
  if (!secret) return; // dev: captcha not configured, skip
  // Fail-closed: if captcha is configured, token is mandatory.
  if (!token || typeof token !== "string") throw new TurnstileError("missing captcha token");
  ...
  try {
    ...
    // Fail-open only on genuine Yandex outage (5xx / network error) — not on missing token.
    if (!res.ok) return;
    data = await res.json().catch(() => ({}));
  } catch {
    return; // network error — fail-open to avoid blocking users on SmartCaptcha outage
  }
...
function validateCourses(courses) {
  ...
  return courses.map((c) => {
    ...
    // Price, flowId, and name are taken from the server-side catalog — never trusted from the client.
    const price = PRICE_CATALOG[productId];
    if (price === undefined) throw new Error(`Unknown productId: ${productId}`);

    const flowId = TARGET_FLOW_ID;
    const name = String(c.name || "Интенсив").replace(/<[^>]*>/g, "").slice(0, 200);
    return { productId, flowId, price, name };
  });
}

function lookupPromo(code) {
  return PROMO_CODES[String(code || "").toUpperCase().trim()] || 0;
}
...
function computeTotal(courses, promoCode) {
  ...
  const discount = PROMO_CODES[String(promoCode || "").toUpperCase().trim()] || 0;
...
module.exports = { isConfigured, getProducts, checkout, mockCheckout, lookupPromo, turnstileSiteKey, TurnstileError };
